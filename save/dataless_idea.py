...
for i in range(calc_cpt_nb-st.session_state['counter']):
    ...
    comp_c[1].number_input("",value=5,step=5,min_value=5,key="cpt n"+str(i))]]


tt=np.array(np.array([list_comp])[0][:,-1],dtype=int)//5-1

st.session_state['counter']= np.sum(tt)
st.text(str(st.session_state['counter'])+" compétences skip")
st.text(tt)
